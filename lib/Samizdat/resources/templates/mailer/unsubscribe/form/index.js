(function () {
  ...
  document.querySelector('#unsubscribeForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    const result = await fetch('<%== url_for("Mailer.unsubscribe.request") %>', {
      method: 'POST',
    ...
    if (result?.success) {
      document.querySelector('#unsubscribeForm').classList.add('d-none');
